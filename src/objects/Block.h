...
public:
    ...
    Block(long l, long w, long h, bool fake)
        : Volume(l, w, h)
        , n_boxes(1)
        , spaces(NULL)
        , blocks(NULL)
...
protected:
    ...
    Block(const Block & b)
        : Volume(b.getL(), b.getW(), b.getH())
        , occupied_volume(b.occupied_volume)
        , n_boxes(b.n_boxes)
        , spaces(new SpaceSet(*b.spaces, *this))
        , blocks(new AABBList(*b.blocks))
