...
Block::Block(double l, double w, double h)
    : Volume(l, w, h)
    , occupied_volume(0)
    , total_weight(0.0)
    , total_profit(0.0)
    , n_boxes(0)
    , spaces(new SpaceSet(*this))
    , blocks(new AABBList()) {
    spaces->insert(Space(*this));
}

Block::Block(const BoxShape & box, BoxShape::Orientation o, double occupied_volume)
    : Volume(box.getL_d(o), box.getW_d(o), box.getH_d(o))
    , occupied_volume(occupied_volume)
    , total_weight(box.get_weight())
    , total_profit(box.get_profit())
    , n_boxes(1)
    , spaces(NULL)
    , blocks(NULL) {
...
list<const Block *> Block::create_new_blocks(const Block * b2, double min_fr, const Vector3 & max_dim, double wmax) const {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        switch (i) {
        case 0:
            ll = (b1->getL_d() + b2->getL_d());
            x2 = b1->getL();
            break;
        case 1:
            ww = (b1->getW_d() + b2->getW_d());
            y2 = b1->getW();
            break;
        case 2:
            hh = (b1->getH_d() + b2->getH_d());
            z2 = b1->getH();
        }

        double vol = ll * ww * hh;

        if (((double)(b1->occupied_volume + b2->occupied_volume) / (double)vol) >= min_fr && Vector3(ll, ww, hh) <= max_dim && (b1->getTotalWeight() + b1->getTotalWeight() <= wmax || wmax == 0.0)) {
