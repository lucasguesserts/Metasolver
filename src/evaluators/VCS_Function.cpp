...
VCS_Function::VCS_Function(map<const BoxShape *, int> & nb_boxes, Vector3 & dims, double alpha, double beta, double gamma, double p, double delta, double delta2, double delta3, double r)
    : VLossFunction(nb_boxes, dims, r)
    , alpha(alpha)
    , beta(beta)
    , gamma(gamma)
    , p(p)
...
double VCS_Function::eval_action(const State & s, const Action & a) {
    ...
    double loss = (beta > 0.0) ? Loss(dynamic_cast<const clpState *>(&s)->nb_left_boxes, b, sp) : 0.0;
    ...
    if (clpState::Wmax > 0.0) {
        ...
        return (pow(vol, delta) * pow((1.0 - loss), beta) * pow(cs, alpha) * pow(n, gamma) * pow(density, delta2) * pow(profit, delta3));
    }
...
double VCS_Function::CS_p(const State & s, const Block & b, const Space & sp, double p) {
    ...
    std::list<const AABB *> blocks = dynamic_cast<const clpState *>(&s)->cont->blocks->get_intersected_objects(bexp);
