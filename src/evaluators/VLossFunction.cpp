...
long VLossFunction::compute_maxX(const std::map<const BoxShape *, int> & nb_boxes, const Block & block, long & lossX, long resX, std::vector<long> & mX, std::set<const BoxShape *> * listX) {
    long maxX = mX[resX];
    while (maxX > 0) {
...
void VLossFunction::compute_mX(const std::map<const BoxShape *, int> & nb_boxes, int X, std::vector<long> & mX, std::set<const BoxShape *> * listX, int dim) {
    bool flag[X + 1];
    flag[0] = true;
